  ...
  const toggleLikeMutation = useMutation({
    mutationFn: async ({ mealId }) => {
      if (!user) {
        ...
        return Promise.reject(new Error('Login Required')); // Reject to prevent onSuccess
      }
      ...
      if (!isPremium) {
        ...
        return Promise.reject(new Error('Premium Membership Required')); // Reject
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['upcoming-meals']); // Invalidate to refetch updated likes
    },
    onError: (error) => {
      console.error("Error toggling like:", error);
      // Swal messages are handled within mutationFn for specific cases
      // Generic error for other failures
      if (!error.message.includes('Login Required') && !error.message.includes('Premium Membership Required')) {
                  ...
                  <div className="flex justify-between items-center mt-auto"> {/* Aligned to bottom */}
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${!isPremium && user ? 'opacity-50 cursor-not-allowed' : ''} {/* Dim if not premium but logged in */}
                      `}
                      disabled={toggleLikeMutation.isPending || (!isPremium && user)} // Disable if mutating or not premium (but logged in)
                      title={!user ? "Login to like" : (!isPremium ? "Upgrade to like" : "")} // Tooltip for disabled state
                    >
                      {toggleLikeMutation.isPending ? 'Processing...' : (
                        hasLiked ? (
